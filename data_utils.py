...
class LMDB_Image:
    def __init__(self, haze, clear, n1): # delete n2, n3, n4, n5, n6
    ...
    def get_image(self):
        ...
        # return haze.reshape(*self.size, self.channels), clear.reshape(*self.size, self.channels), np.resize(n1, (*self.size,
        ...
        return haze.reshape(*self.size, self.channels), clear.reshape(*self.size, self.channels), np.resize(n1, (*self.size,self.channels))                                                         
        # FLAG: for n1, instead of using reshape, utilize resize because of (256,256,3) impossible converted to (512,512,3)
...
#     dataset=DatasetLMDB(os.path.join(path, 'ITS.lmdb'), size=crop_size), batch_size=BS,
#     shuffle=True, pin_memory=True) #FLAG
...
# SATE1K_Thick_train_loader_all = DataLoader(
#     dataset=DatasetLMDB(os.path.join(path, 'SATE1K_Thick/SATE1K_Thick.lmdb'), size=crop_size), batch_size=BS,
#     shuffle=True, pin_memory=True)
SATE1K_Thick_train_loader_all = DataLoader(
    dataset=DatasetLMDB('/kaggle/input/data-lmdb/SATE1K_Thick.lmdb', size=crop_size), batch_size=BS,
    shuffle=True, pin_memory=True)
SATE1K_Thick_test_loader = DataLoader(
    dataset=RESIDE_Dataset('/kaggle/input/sate1k-thick/dataset/test', train=False, size='whole img', format='.png'),
    batch_size=1,
    shuffle=False)
