...
loaders_ = {
    ...
    # 'rice_test': RICE_test_loader,
    # 'rice_train': RICe_train_loader_all,
    # 'rice_test': RICE_test_loader,
    # 'rice_train': RICe_train_loader_all,
    'SATE1K_Thick_test': SATE1K_Thick_test_loader,
    'SATE1K_Thick_train': SATE1K_Thick_train_loader_all ,
}
...
def clcr_train(train_model, train_loader, test_loader, optim, criterion):
    ...
    if opt.resume and os.path.exists(opt.model_dir):
        ...
        print(
            f'n1_weight:{weights[0]}| inp_weight:{weights[1]}')
    ...
    for step in range(start_step + 1, opt.steps + 1):
        ...
        x, y, n1, inp = next(iter(train_loader))
        x = x.to(opt.device)
        y = y.to(opt.device)
        n1 = n1.to(opt.device)
        # n2 = n2.to(opt.device)
        ...
        if opt.clcrloss:
            ...
            loss2 = criterion[1](out, y, n1, x, weights)
